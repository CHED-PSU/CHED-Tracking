...
export default function ReturnRequest(props) {
    ...
    const [itemDataRII, setItemDataRII] = useState();
    const [itemDataURI, setItemDataURI] = useState();
    ...
    useEffect(() => {
        const getItemRequestData = async () => {
            ...
            try {
                await axios
                    .post("/api/getItemRequestData", {
                        ui_id: props.valueId,
                    })
                    .then((res) => {
                        setItemData(res.data.itemData);
                        setItemDataRII(res.data.itemDataRII);
                        setItemDataURI(res.data.itemDataURI);
                    });
            } catch (e) {
                console.log(e);
            } finally {
    ...
    const success = (data) => {
        if (data === "success") {
            ...
            setAlertDesc(
                "You've successfuly returned an item to its previous owner."
            );
            setAlertButtonColor("none");
    ...
    return (
        <div>
            ...
            <div className="z-30 w-full h-full bg-neutral-800 bg-opacity-75 fixed top-0 right-0 flex justify-center items-center">
                <div
                    ref={modalBody}
                    className="bg-white w-[700px] p-8 space-y-6 rounded-2xl flex flex-col items-left"
                >
                    ...
                    <div className="flex justify-between">
                        <div className="w-1/2 space-y-1">
                            <h6 className="text-[18px] pb-2">
                                Description of Property
                            </h6>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">Type:</h6>
                                <h5 className="underline">
                                    {itemData ? itemData.article : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">Serial No:</h6>
                                <h5 className="underline">
                                    {itemData ? itemData.serial_no : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <h5 className="underline">
                                    P{itemData ? itemData.acquisition : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">
                                ...
                                <h5 className="underline">
                                    {itemData
                                        ? itemDataRII.pre_nature == null
                                            ? "Not yet repaired."
                                            : itemDataRII.pre_nature
                                        : "N/A"}
                                </h5>
                            </div>
                        </div>
                        <div className="w-1/2 space-y-1">
                            <div className="flex gap-2">
                                ...
                                <h5 className="underline">
                                    {itemData ? itemData.tracking_id : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">Brand/Model:</h6>
                                <h5 className="underline">
                                    {itemData ? itemData.brand : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">Property No:</h6>
                                <h5 className="underline">
                                    {itemData ? itemData.property_no : "N/A"}
                                </h5>
                            </div>
                            <div className="flex gap-2">
                                <h6 className="font-semibold ">
                    ...
                    <div>
                        ...
                        <input
                            onKeyUp={bbdefectHandler}
                            type="textarea"
                            className=" border border-neutral-500 rounded-lg w-full p-4 outline-none"
                        />
                    </div>
                    <div className="flex flex-col justify-between">
                        ...
                        <select
                            ...
                            name="purpose"
                            id="Status"
                            className="w-full rounded-md border border-neutral-500 p-4 outline-none"
                        >
                            <option value="None">None</option>
                            <option value="Return for Repair">
                                Return for Repair
                            </option>
                            <option value="Return for Disposal">
                        ...
                        {isOther ? (
                            <>
                                <label
                                    htmlFor="textareaopt"
                                    className="text-base font-semibold pt-6"
                                >
                                    Other
                                </label>
                                <input
                                    onKeyUp={bbcommenthandler}
                                    type="textarea"
                                    id="textareaopt"
                                    className=" border border-neutral-500 rounded-lg w-full p-4 outline-none"
                                />
                    ...
                    <div className="flex gap-3 w-full justify-end">
                        <button
                            onClick={props.openFormHandler}
                            className="btn-color-5 text-white rounded-full px-4 py-2 font-medium"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={clickAlert}
                            value={itemData ? itemData.id : "N/A"}
                            className="btn-color-4 text-white rounded-full px-4 py-2 font-medium"
                        >
                            Send
                        </button>
                    </div>
